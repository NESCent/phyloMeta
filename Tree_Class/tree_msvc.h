//modified such that file extention is now h not hh
//MArc Lajeunesse added function subtree(sibling iterator) function that returns a subtree starting at sibling iterator
//
/* 
...
class tree {
   ...
   public:
	  ...
	  ///Marc Lajeunesse: creates a subtree rooted with the called sibling_iterator
      tree     subtree(sibling_iterator from) const
         {
         tree tmp;
         tmp.set_head(value_type());
         tmp.replace(tmp.begin(), from);
         return tmp;
         }
